...
def find_cheeky_links(dir: str) -> bool:
    ...
    link_pattern_target_blank = re.compile(
        r'\[([^\]]+)\]\((https?://[^\)]+)\)(?!\{\:target="_blank"\})'
    )
    link_pattern_icon = re.compile(
        r"\[([^\]]+?)(?<!\:octicons\-link\-external\-16\:\{\s\.external\-link\-icon\s\})\]\((https?:\/\/[^\)]+)\)"
    )
    matches_found = False
    ...
    for root, _, files in os.walk(dir):
        for file in files:
            if file.endswith(".md"):
                ...
                # Ignore image links
                matches_target_blank = [
                    match
                    for match in matches_target_blank
                    if "raw.githubusercontent.com" not in match[1]
                    and "https://github.com/theselftaughtdev42.png" not in match[1]
                ]
                matches_icon = [
                    match
